package application;

import org.eclipse.paho.client.mqttv3.*;

import java.util.ArrayList;
import java.util.List;

public class MovementDetection {

    private MqttClient client;
    public List<MovementSensor> allSensors = new ArrayList<>();
    public List<MovementSensor> moveRoom = new ArrayList<>();
    public boolean movement;
    public String currentRoom;

    public MovementDetection() {
        String clientId = MqttClient.generateClientId();

        try {
            client = new MqttClient(Topics.BROKER_URL, clientId);

        } catch (MqttException e) {
            e.printStackTrace();
            System.exit(1);
        }
    }

    public void main() {
        MovementSensor ms1 = new MovementSensor("Livingroom");
        MovementSensor ms2 = new MovementSensor("Bedroom");
        MovementSensor ms3 = new MovementSensor("Bathroom");
        MovementSensor ms4 = new MovementSensor("Kitchen");

        allSensors.add(ms1);
        allSensors.add(ms2);
        allSensors.add(ms3);
        allSensors.add(ms4);

        start();


    }

    public void monitorSensors() {

        moveRoom.clear();
        int noMove = 0;

        for (int i = 0; i < allSensors.size(); i++) {
            if (allSensors.get(i).getSensorData() == 0) {
                noMove = noMove + 1;
            } else if (allSensors.get(i).getSensorData() == 1) {
                movement = true;
                currentRoom = allSensors.get(i).sensorName;
            }
        }

        if (noMove == allSensors.size()) {
            System.out.println("No movement in house! Send ALARM");
            movement = false;
        }

    }

    public void start() {
        try {
            MqttConnectOptions options = new MqttConnectOptions();
            options.setCleanSession(false);

            client.connect(options);


            while (true) {

                monitorSensors();

                if(!movement){
                    sendSafety();
                }
                sendSecurity(currentRoom);


            }
        } catch (MqttException | InterruptedException e) {
            e.printStackTrace();
            System.exit(1);
        }


    }

    public void sendSecurity(String room) throws InterruptedException {
        final MqttTopic security = client.getTopic(Topics.MOVEMENT_SECURITY);

        String finalSafeMsg = room + "/true";
        Thread thread = new Thread() {
            public void run() {
                try {
                    security.publish(new MqttMessage(finalSafeMsg.getBytes()));
                } catch (MqttException e) {
                    e.printStackTrace();
                }
            }
        };
        thread.start();

        System.out.println("Published data. Topic: " + security.getName() + "  Message: " + finalSafeMsg);

        thread.sleep(500);

    }

    public void sendSafety() throws InterruptedException {
        final MqttTopic safety = client.getTopic(Topics.MOVEMENT_SAFETY);

        String finalSafeMsg = "movement/false"; //movement/false;
        Thread thread = new Thread() {
            public void run() {
                try {
                    safety.publish(new MqttMessage(finalSafeMsg.getBytes()));
                } catch (MqttException e) {
                    e.printStackTrace();
                }
            }
        };
        thread.start();

        System.out.println("Published data. Topic: " + safety.getName() + "  Message: " + finalSafeMsg);

        thread.sleep(500);

    }
    }

